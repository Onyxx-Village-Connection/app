...
class Volunteer {
  ...
  sortVolunteerDeliveries() {
    volunteerDeliveries
        .sort((a, b) => a.getDateInt().compareTo(b.getDateInt()));
  }
  ...
  addPickup(Pickups pickup) {
    ...
    for (int i = 0; i < volunteerPickups.length; i++) {
      if (volunteerPickups[i].item == pickup.item &&
          volunteerPickups[i].date == pickup.date) {
        counter++;
      }
    }
    if (counter == 0) {
  ...
  addDelivery(Deliveries deliveries) {
    ...
    for (int i = 0; i < volunteerDeliveries.length; i++) {
      if (volunteerDeliveries[i].item == deliveries.item &&
          volunteerDeliveries[i].date == deliveries.date) {
        counter++;
      }
    }
    if (counter == 0) {
